...
### 5. Confidence-скоринг и gate

Итоговая уверенность складывается из базы правила + контекстных бустов/штрафов
(`internal/context`, ±100 байт вокруг span: `+0.2` за каждое слово-усилитель,
`−0.3` за слово-штраф). Порог `Gate = 0.95`: ниже — не маскируем. Одиночный
«чувствительный» тип (`ПИН`, `CVV`) маскируется только при наличии другой ПД
рядом (co-occurrence).

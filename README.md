...
| **3 режима маскирования** | `redact` (`[КЛАСС]`), `token` (`[LABEL_NNN]` с картой `token→значение`), `synthetic` |
...
| **Per-system конфигурация** | Каждая система-потребитель: свои типы ПД, режим маски, право на демаскирование |
| **Гибкая авторизация** | `/process` доступен без токена; неверный токен → 401; `require_key` принудительно требует ключ |
| **Overlay-правила** | Новые типы ПДН (СНИЛС и т.п.) добавляются через конфиг без правки ядра |
| **Комбинации типов** | ПИН/CVV маскируются только при наличии другой ПД рядом (co-occurrence) |
| **Админка (SPA)** | Управление командами, типами ПД, ключами, режимами; примеры запросов с копированием |
| **Метрики + Grafana** | Prometheus-метрики, дашборд «PII Gateway» (RPS, latency, PII по типам, Go runtime) |
| **High-load** | RPS 40k+ на коротких текстах, параллельная детекция ≥4096 байт, in-memory LRU store |

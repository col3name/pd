...
func TestPoolProcessesJob(t *testing.T) {
	p := New(2, 16, 8, func(system, payload string) Result {
		return Result{Masked: "masked:" + system + ":" + payload}
	})
	defer p.Close()
	res, err := p.Submit(context.Background(), "sys", "hello", false)
	require.NoError(t, err)
	require.Equal(t, "masked:sys:hello", res.Masked)
}

func TestPoolBackpressure(t *testing.T) {
	// Handler blocks forever; fill the fast queue, then Submit must time out.
	block := make(chan struct{})
	p := New(1, 2, 2, func(system, payload string) Result {
		<-block
		return Result{}
	})
	defer p.Close()
...
func TestPoolMetrics(t *testing.T) {
	...
	defer p.Close()
	_, _ = p.Submit(context.Background(), "sys", "x", false)
...
func TestPoolHandlerError(t *testing.T) {
	p := New(1, 4, 4, func(system, payload string) Result {
		return Result{Err: errors.New("boom")}
	})
	defer p.Close()
	res, err := p.Submit(context.Background(), "sys", "x", false)
	require.NoError(t, err)
